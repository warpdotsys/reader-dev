 ...
 */
function showTip(title: string, closeText: string, bodyHtml: string, extraClass?: string) {
    $("#tipName").html(title);
...
$("#dropdown-backdrop").click(function (e: any) { closeTip(); });
...
var Title = function () {
    ...
    this.set = function (t: string) { document.title = t; };
    this.reset = function () { document.title = this.title; };
    this.update = function (suffix: string) { document.title = this.title + " - " + suffix; };
};
...
var token: string = $.getUrlPra("accessToken");
if (token) {
...
$.ajax.onResponse = function (responseText: string) {
    try {
 ...
 */
function requireTmpl(name: string, callback: any) {
    window.__TMPL__ = window.__TMPL__ || {};
    if (window.__TMPL__[name]) {
        callback(window.__TMPL__[name]);
    } else {
        $.ajax.get("./assets/template/" + name + ".tmpl", function (tmplStr: string) {
            callback(tmplStr);
        }, true);
    }
}
 ...
 */
function renderTmpl(name: string, data: any, callback: any) {
    requireTmpl(name, function (tmpl: any) {
        var html;
 ...
 */
function Watcher(initialValue?: any) {
    var listeners = [];
     ...
     */
    this.onChange = function (fn: any, immediate?: boolean) {
        if (fn) {
            listeners.push(fn);
            if (immediate) {
    ...
    this.update = function (newValue: any) {
 ...
 */
function setBookSource(bookUrl: string, index: number) {
 ...
 */
function loadMoreBookSource(bookUrl: string) {
    ...
    $.ajax.post("/searchBookSource", {
        ...
        try {
            ...
            if (result.isSuccess) {
                ...
                var seen: any = {};
                existing.map(function (item: any) { seen[item.bookUrl] = 1; });
                var merged = [].concat(existing, newItems.filter(function (item: any) {
 ...
 */
function submitForm(form: any, transform: any, callback: any) {
    var data: any = {};
    ...
    if (data) {
        $.ajax.post(form.getAttribute("action"), data, function (resp: string) {
            try {
                var result = JSON.parse(resp);
...
function doLogin(form: any, isLogin: boolean) {
    submitForm(form, function (data: any) {
...
function doLogout() {
    $.ajax.post("/logout", "", function (resp: string) {
        try {
            var result = JSON.parse(resp);
...
isLoginWatcher.onChange(function (loggedIn: boolean) {
...
function toggleScrollBarHidden(el: any) {
 ...
 */
function chooseMenu(element: any, sectionId: string) {
...
function loadUserInfo() {
    $.ajax.get("/getUserInfo", function (resp: string) {
        var result = JSON.parse(resp);
        if (result.isSuccess) {
            console.log(result);
        }
...
window.onload = function () {
    updatePagePosition();
    window.__onMountedHook.forEach(function (fn: any) { return fn(); });
};
...
var BookApi = function () {
    ...
    this.updateBook = function (book: any) {
    ...
    this.getBookshelf = function (callback?: any) {
        $.ajax.get("/getBookshelf", function (resp: string) {
            var result = JSON.parse(resp);
            if (result.isSuccess) {
    ...
    /** Find a book by URL */
    this.getBookInfoByUrl = function (url: string) {
        var list = bookListWatcher.getValue();
        for (var i = 0; i < list.length; i++) {
    ...
    this.find = function (index: number) {
        return (index >= 0 && index < this.book.length) ? index : -1;
    };

    this.isFavourite = function (index: number) {
        return this.find(index) === -1 ? "收藏本书" : "取消收藏";
    };

    this.favourite = function (index: number) {
    ...
    this.getData = function (index: number) {
...
var Pagination = function () {
     ...
     */
    this.init = function (itemSelector: string, offset: number, containerSelector?: string, onPageChange?: any, initialPage?: number) {
    ...
    this.set = function (which: string, className: string) {
    ...
    this.display = function (page: number) {
...
var Menu = function () {
     ...
     */
    this.generateMenu = function (isLogin: boolean) {
    ...
    this.initMenu = function () {
        ...
        isLoginWatcher.onChange(function (isLogin: boolean) { self.generateMenu(isLogin); }, true);
        window.darkThemeWatcher.onChange(function (dark: any) { self.generateMenu(dark); });

        var menu = $("#Menu");

        // Toggle dropdown open/close
        $("#Menu_button").click(function (e: any) {
            var dropup = $("#dropup2");
        ...
        $("#dropdown-backdrop").click(function (e: any) {
        ...
        $("#Real_Menu").on("click", "li", function (e: any) {
