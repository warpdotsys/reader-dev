 ...
 */
function createSignal(initialValue?: any, options?: any) {
...
function writeSignal(state: any, newValue: any) {
    if (state.comparator && state.comparator(state.value, newValue)) {
        return newValue;
    }
    state.value = newValue;
    state.subscribers.forEach(function (fn: any) { return fn(); });
    return newValue;
}
...
var searchConfig: any = null;
var bookSourceGroupList: any = [];
...
function onSearchBookListPageChange(page: number) {
...
function searchBookWithSourceIndex(index: number) {
    closeTip();
    searchBook($("#keyword").val(), index);
}

function searchBook(keyword: string, sourceIndex?: number) {
...
function searchBookWithConfig(config: any) {
    ...
    $.ajax.post(config.isMulti ? "/searchBookMulti" : "/searchBook", {
        ...
        var byNameAuthor: any = {};
        var byUrl: any = {};
        var prevLength = existing.length;

        existing.reduce(function (acc: any, book: any) {
            acc[book.name + "_" + book.author] = book;
            byUrl[book.bookUrl] = book;
            return acc;
        }, byNameAuthor);

        newItems.forEach(function (book: any) {
 ...
 */
function getSearchConfig(keyword: string, sourceIndex?: number) {
    ...
    if (keyword.indexOf("#") > 0) {
        ...
        var matchedGroups = bookSourceGroupList.filter(function (g: any) {
        ...
        var matchedSources = bookSourceList().filter(function (s: any) {
...
function showBookInfo(index: any) {
    var book = searchBookList()[index];
    if (!book) {
        book = searchBookList().find(function (b: any) { return b.bookUrl === index; });
    }
    if (!book) return;

    var shelfBook = bookShelfList().find(function (b: any) { return b.bookUrl === book.bookUrl; });
...
function saveBook() {
    ...
    $.ajax.post("/saveBook", info, function (resp: string) {
        var result = JSON.parse(resp);
        if (result.isSuccess) {
            showNotice("\u52a0\u5165\u6210\u529f");
...
function refreshChapterList(bookUrl: string) {
    var book = searchBookList().find(function (b: any) { return b.bookUrl === bookUrl; });
    if (!book) return;

    $.ajax.post("/getChapterList", { url: book.bookUrl, bookSourceUrl: book.origin }, function (resp: string) {
        var result = JSON.parse(resp);
        if (result.isSuccess) {
            setBookInfo(function (info: any) {
...
function changeSource(bookUrl: string) {
    var book = searchBookList().find(function (b: any) { return b.bookUrl === bookUrl; });
    if (!book) return;

    sourceListWatcher = new Watcher();
    sourceListWatcher.onChange(function (sources: any) {
        if (sources === "loading") {
            ...
            renderTmpl("sourceList", { bookUrl: bookUrl, list: sources }, function (html: string) {
                showTip(book.name + "(" + book.originName + ")", "\u5173\u95ed",
                    "<div style='flex: 1'>" + html + "</div>");
            });
        } else {
    ...
    $.ajax.post("/getAvailableBookSource", { url: bookUrl }, function (resp: string) {
        try {
            var result = JSON.parse(resp);
            if (result.isSuccess) {
...
function loadBookSourceList() {
    $.ajax.get("/getBookSources?simple=1", function (resp: string) {
        var result = JSON.parse(resp);
        if (!result.isSuccess) return;
        ...
        // Build group list
        var groupCounts: any = {};
        result.data.forEach(function (source: any) {
            if (source.bookSourceGroup) {
                source.bookSourceGroup.split(",").forEach(function (group: string) {
...
function loadBookShelfList(callback?: any) {
...
onMounted(function () {
    ...
    $("#keyword").on("keyup", function (e: any) {
