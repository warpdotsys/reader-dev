...
`,
  ...
  methods: {
    ...
    loadBookGroup(refresh?: any) {
      return this.$root.$children[0].loadBookGroup(refresh);
    },
    getShowShelfBooks(bookGroup: any) {
      // 处理特殊分组
      if (bookGroup === -1) {
        // 全部
    ...
    toggleBookGroupShow(bookGroup: any, show: any) {
    ...
    async saveBookGroup(bookGroup?: any) {
    ...
    displayBookGroupName(bookGroup: any) {
      return (
        bookGroup.groupName +
        (bookGroup.groupId < 0
          ? "(" +
    ...
    getBookGroupListForBook(bookGroup: any) {
      const groups = [];
