// Generated from ShadowIframe.vue
...
const traditionalized = (text: any) => {
...
export const ShadowIframe = {
  ...
  watch: {
    ...
    renderIframe(value: boolean) {
      if (!value) {
        this.loadShadowDom(this.nowSrc || this.src);
      }
    },
    chineseFont(value: string) {
  ...
  methods: {
     ...
     */
    htmlLoader(html: string, baseUrl: string) {
      let result = html
    ...
    async loadShadowDom(url: string) {
      if (this.initing || !url) {
        return;
      }
      this.initing = true;
    ...
    renderShadowDom(html: string, url: string) {
    ...
    initShadowDomJavascript(url: string) {
      const self = this;
    ...
    sendToIframe(event: string, data: any) {
      if (
        this.$refs.iframe &&
    ...
    ensure(callback: any, condition: any) {
