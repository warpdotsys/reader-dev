...
- 当前对外前端仍是 Vue 2。新界面应在独立目录开发，使用独立预览入口；不得直接覆盖 `web/`、JAR 原版静态资源或生产入口。
...
**已从源码验证**：Rust 前端的设计系统和请求约定存在；Java/Kotlin 注册了部分同名路由。**尚未验证**：两者端到端兼容、缺失接口的替代方案、子目录部署和生产可用性。**尚未实施**：独立 Vue 3 工程、页面迁移和默认入口切换。
