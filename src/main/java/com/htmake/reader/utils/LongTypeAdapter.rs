...
/**
 * Gson TypeAdapter for Long that safely handles None, empty string, and floating point values.
 */
...
impl JsonDeserializer<i64> for LongTypeAdapter {
    ...
    //     return if (primitive.isNumber) primitive.asNumber.toLong() else None
    // }
    fn deserialize(&self, json: JsonElement, type_of_t: Option<Type>, context: Option<JsonDeserializationContext>) -> Option<i64> {
        ...
        return if primitive.is_number() { Some(primitive.as_number().to_long()) } else { None };
    }
}
